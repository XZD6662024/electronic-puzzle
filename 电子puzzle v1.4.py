...
def get_feedback_hard(secret_code, guess):
    """根据玩家的猜测和密码，给出反馈（困难模式）"""
    feedback = []
    for i in range(len(guess)):
        if guess[i] == secret_code[i]:
            feedback.append('绿')  # 颜色和位置都正确
...
def introduce():
    ...
    print("7. 在游戏过程中，你可以随时输入“切换模式”来修改简易程度。")
    print("=" * 50)
    print("开发者：XZD")
    print("版本：1.4")
    print("=" * 50)
...
def main():
    ...
    while attempts < max_attempts:
        attempts += 1
        print(f"\n第 {attempts} 次尝试：")
        guess = input("请输入4种颜色（用空格分隔），或输入“答案”查看正确答案，或输入“切换模式”修改简易程度：").strip().split()
        ...
        # 检查玩家是否想切换模式
        if len(guess) == 1 and guess[0].lower() == '切换模式':
            mode = '困难' if mode == '简单' else '简单'
        ...
        if mode == '简单':
            ...
            feedback = get_feedback_hard(secret_code, guess)
